...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("Back", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"Page {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('Source', url='https://t.me/wlmlsource')
            ],[
            InlineKeyboardButton('Home', callback_data='start'),
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("Sorry, Nothing found. Check your spelling. or search again with any of the following suggestions. DON'T CLICK, THOSE ARE NOT BUTTONS.Type it and send it to me. Add or remove spaces for better results.", reply_markup=InlineKeyboardMarkup(btn))
